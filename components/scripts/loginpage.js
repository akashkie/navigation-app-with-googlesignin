 ...
 const App = () => {
  ...
  const googleLogin = async () => {
   ...
   const isSignedIn = async () => {
    ...
    console.log({ isLoginScreenPresented: !isSignedIn });
  };
    ...
    try {
      ...
      console.log( "user: null "); // Remember to remove the user from your app's state as well
    } catch (error) {
      console.error(error);
    }
  };
  ...
  return (
    <View style={{flex: 1, justifyContent: 'center', alignItems: 'center',backgroundColor:"lightblue"}}>
      ...
      <TouchableOpacity style={styles.Container} 
         onPress={ isSignedIn ? googleLogin : signOut} >
           <Image source={require('./images/google.png')} style={styles.Img} />
        <Text style={styles.Text}> { !isSignedIn ? ' Signin with Google' : '      Log out           '} </Text>
        </TouchableOpacity>
